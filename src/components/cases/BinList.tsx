...
export const BinList = ({ deletedCases, onRestore, onPermanentDelete, onEmptyBin, onRestoreAll }: BinListProps) => {
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === deletedCases.length) setSelectedIds(new Set());
    else setSelectedIds(new Set(deletedCases.map(c => c.id)));
  };
          ...
          {selectedIds.size > 0 && (
            <>
              <Button variant="outline" size="sm" onClick={handleBatchRestore}>
                <RotateCcw className="mr-2 h-5 w-5" /> Restore {selectedIds.size} selected
              </Button>
              <Button variant="destructive" size="sm" onClick={() => setDeleteDialogOpen(true)}>
                <Trash2 className="mr-2 h-5 w-5" /> Delete permanently
              </Button>
            </>
          )}
          {deletedCases.length > 0 && selectedIds.size === 0 && (
            <>
              <Button variant="outline" size="sm" onClick={onRestoreAll}>
                <RotateCcw className="mr-2 h-5 w-5" /> Restore All
              </Button>
              <Button variant="destructive" size="sm" onClick={() => setEmptyBinDialogOpen(true)}>
                <Trash2 className="mr-2 h-5 w-5" /> Empty Bin
              </Button>
            </>
          )}
        </div>
          ...
          <TableHeader>
            <TableRow className="bg-muted/50">
              <TableHead className="w-[50px]">
                <Checkbox checked={allSelected} onCheckedChange={toggleSelectAll} aria-label="Select all" disabled={deletedCases.length === 0} />
              </TableHead>
              <TableHead className="font-semibold">Case Number</TableHead>
              <TableHead className="font-semibold">Title</TableHead>
              <TableHead className="font-semibold">Garnishee</TableHead>
            ...
            {deletedCases.length === 0 ? (
              ...
              deletedCases.map((caseItem) => (
                ...
                >
                  ...
                  <TableCell className="text-muted-foreground">{formatDate(caseItem.deletedAt)}</TableCell>
                  <TableCell>
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" onClick={() => onRestore(caseItem.id)}>
                        <RotateCcw className="h-4 w-4" />
                      </Button>
                      <Button variant="destructive" size="sm" onClick={() => setSingleDeleteId(caseItem.id)}>
      ...
      <AlertDialog open={!!singleDeleteId} onOpenChange={(open) => !open && setSingleDeleteId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => { if (singleDeleteId) { onPermanentDelete(singleDeleteId); setSingleDeleteId(null); } }}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete all {deletedCases.length} cases in the bin.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => { onEmptyBin(); setEmptyBinDialogOpen(false); }} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
              Empty Bin
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
